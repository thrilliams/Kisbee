...
abstract class Channels {
	static initChannelMap(min?: number, max?: number): ChannelMap {
	...
	@Slash('verify', {
		description: 'Verfies the existance of channels for each applicable subject.',
		defaultPermission: false
	})
	async verify(
		@SlashOption('verbose', { description: 'Display additional information.' })
		verbose: boolean,
		...
		let success = true;
		embed
			.setTitle('Verifying channels, please wait...')
		...
		let totalFound = channelMap.reduce(
			(total, group) => (total += group.channel !== undefined ? 1 : 0),
			0
		);
		embed.fields.find((field) => field.name === 'Channel names:')!.value = `${totalFound}/${
			channelMap.size
		} found${
			verbose
				? `:\n${channelMap
						.filter((group) => group.channel !== undefined)
						.map((group) => `#${group.cleanedName}`)
						.join('\n')}`
				: '.'
		}`;
		...
		embed.fields.find((field) => field.name === 'Channel permissions:')!.value = `${
			validChannelMap.size
		}/${totalFound} correct${
			verbose
		...
		embed.setTitle(
			success
				? '✅ Subject channels successfully verified.'
				: '⚠️ Subject channel verification failed! Please adress issues below.'
		);
		await interaction.editReply({ embeds: [embed] });
	}
	...
	@Guard(HasPermission('MANAGE_CHANNELS'))
	@Slash('create', {
		description: 'Creates channels as needed for each applicable subject.',
		defaultPermission: false
	})
	async create(interaction: CommandInteraction) {
		let embed = new MessageEmbed();
		embed
			.setTitle('Creating channels, please wait...')
			.addField('Channel names:', '...')
			.addField('Channel permissions:', '...');
		await interaction.reply({ embeds: [embed] });
		...
		embed.fields.find(
			(field) => field.name === 'Channel names:'
		)!.value = `${totalFound}/${channelMap.size} found.`;
		if (totalFound < channelMap.size) {
			embed.addField(
				'Creating channels:',
				channelMap
		...
		embed.fields.find(
			(field) => field.name === 'Channel permissions:'
		)!.value = `${validChannelMap.size}/${channelMap.size} correct.`;
		if (validChannelMap.size < channelMap.size) {
			...
			embed.addField(
				...
				invalidChannelMap.map((group) => `#${group.cleanedName}`).join('\n')
			);
			await interaction.editReply({ embeds: [embed] });
			for (let [name, group] of invalidChannelMap) {
				...
				await group.channel!.permissionOverwrites.set([
					...
					{
						id: interaction.guild!.roles.cache.find((role) => role.name === 'Helper')!
							.id,
						allow: ['VIEW_CHANNEL']
					},
					{
	...
	@Guard(HasPermission('MANAGE_CHANNELS'))
	@Slash('archive', {
		description: 'Archive a channel into a text file.',
		defaultPermission: false
	})
	async archive(
		@SlashOption('forcedelete', { description: 'Delete the channel after archiving it.' })
		forcedelete: boolean,
		interaction: CommandInteraction
	) {
		...
		if (forcedelete === true) {
			...
			let message = interaction.user.send({
				content: `Channel "#${name}" archived, deleting...`,
				files: [attachment]
			});
	...
	@Guard(HasPermission('MANAGE_CHANNELS'))
	@Slash('delete', {
		description: 'Deletes existing channels for each applicable subject.',
		defaultPermission: false
	})
